	...
	function initializeSearch() {
		...
		function goToLatLon(lat, lon) {
			if (lat < -90 || lat > 90 || lon < -180 || lon > 180) {
				M.toast({
					html: "Latitude must be between −90 and 90; longitude between −180 and 180.",
					displayLength: 4000,
				});
				return;
			}
			var center = [lon, lat];
			var zoom = Math.max(map.getZoom(), 12);
			var run = function () {
			...
			M.toast({
				html: "Centered map on " + lat.toFixed(6) + ", " + lon.toFixed(6),
				displayLength: 2500,
			});
		}

		function refreshSearchModeUi() {
			var mode = $('input[name="search-mode"]:checked').val();
			if (mode === "coords") {
				$("#search-place-fields").hide();
				$("#search-coords-fields").show();
			} else {
				$("#search-place-fields").show();
				$("#search-coords-fields").hide();
			}
		...
		$('input[name="search-mode"]').on("change", refreshSearchModeUi);
		refreshSearchModeUi();

		$("#lat-box, #lon-box, #location-box").on("keydown", function (ev) {
		...
		$("#search-form").submit(function (e) {
			...
			var mode = $('input[name="search-mode"]:checked').val();
			if (mode === "coords") {
				var lat = parseCoordInput($("#lat-box").val());
				var lon = parseCoordInput($("#lon-box").val());
				if (isNaN(lat) || isNaN(lon)) {
					M.toast({ html: "Enter valid latitude and longitude (numbers).", displayLength: 3500 });
					return;
				}
				goToLatLon(lat, lon);
			} else {
				var trimmed = String($("#location-box").val() || "").trim();
				if (!trimmed) {
					M.toast({ html: "Enter a place name or switch to coordinates.", displayLength: 3500 });
					return;
				}
				geocoder.query(trimmed);
			}
		});
	}
